...
void mm_cb (dtype *C, dtype *A, dtype *B, int N, int K, int M)
{
  ...
  /* =======================================================+ */
	...
	int temp_a[b_size][b_size], temp_b[b_size][b_size], temp_c[b_size][b_size];
	
	for(int i = 0; i < N; i += b_size) {
		
		for(int j = 0; j < M; j += b_size) {
			
			for(int k = 0; k < K; k += b_size) {
				
				for(x = i ; x < min(i +b_size, N) ; x++ ){
					
					for(y = j; y < min(j + b_size, M) ; y++){
						
						sum = C[x*M + y];
						for(z = k; z < min(k + b_size, K) ; z++){
...
void mm_sv (dtype *C, dtype *A, dtype *B, int N, int K, int M)
{
  ...
  /* =======================================================+ */
	...
	double sum;
	int b_size = 8;
	__m128d a_temp, b_temp, c_temp, inter ;
	/* __m128d b_t;
	__m128d inter;
	__m128d c_t; */
		...
		for(int i = 0; i < N; i ++ ) {
			...
			for(int k = 0; k < K; k += b_size) {
				...
				//for(x = i ; x < min(i +b_size, N) ; x++ ){
					...
					for(y = j; y < j + b_size ; y+=2){
						
						c_temp = _mm_load_pd(C + (i * M + y));
						//sum = C[x*M + y];
						
						for(z = k; z < k + b_size ; z++){
						
							a_temp = _mm_set1_pd(A[i * K + z]);
							b_temp = _mm_load_pd(B + (z * M + y));
							inter = _mm_mul_pd(a_temp, b_temp);
							c_temp = _mm_add_pd(c_temp, inter);
							//sum += A[x * K + z] * B[z * M + y];
						
						}
						
						_mm_store_pd(C + (i * M + y), c_temp);
						//C[x * M + y] = sum; 
					}
				//}		
			}	
